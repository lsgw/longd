...
static int json_decode_from_lightuserdata(lua_State* L)
{
...
static int json_encode_to_lightuserdata(lua_State* L)
{
	try{
		Encoder encode(L, 2);
		StringBuffer s;
		...
		memcpy(data, s.GetString(), s.GetSize());
		lua_pushlightuserdata(L, data);
		lua_pushinteger(L, s.GetSize());
		return 2;
	}
	catch (...) {
		luaL_error(L, "error while encoding");
...
static const luaL_Reg methods[] = {
	// string <--> lua table
	{ "encode_to_lightuserdata", json_encode_to_lightuserdata },
	{ "decode_from_lightuserdata", json_decode_from_lightuserdata },
